<x-app-layout>
    <div class="my-5 ml-10 sm:my-5 sm:mx-5 py-[2px] bg-yellow-500 sm:w-[20%] text-center shadow-md rounded-md">
        <h3 class="font-bold text-2xl uppercase">Edit Spp</h3>
        <span class="font-semibold text-slate-600 underline">{{ $spp->tahun}} - {{ toRupiah($spp->nominal)}}</span>
    </div>
    <form class="w-[80%] sm:w-full sm:max-w-lg mb-5 mx-12 sm:mx-0" method="POST" action="{{ url('admin/cspp/'. $spp->id)}}">
        @method('PUT')
        @csrf
        <div class="md:flex md:items-center mb-6">
          ...
          <div class="md:w-2/3">
            <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500 shadow-md" id="tahun" name="tahun" type="date" value="{{ $spp->tahun}}">
        ...
        <div class="md:flex md:items-center mb-6">
          ...
          <div class="md:w-2/3 flex rounded-md shadow-md">
            ...
            <input class="bg-gray-200 appearance-none border-r-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="nominal" name="nominal" type="text" value="{{ $spp->nominal}}">
